...
- `POST /api/v1/auth/register` — Create a new account (basic per-IP+email throttling enabled)
- `POST /api/v1/auth/login` — Obtain access token (basic per-IP+email throttling enabled)
...
- Auth throttling is process-local in-memory; use a shared edge/API gateway limiter for multi-instance production.
